...
@section('page_title', 'Masters - Add Supplier')

@section('masters_content')
<h1 class="h5 mb-3">Add Supplier</h1>

    <div class="card border-0 shadow-sm">
        <div class="card-body">
            <form method="POST" action="{{ route('masters.suppliers.store') }}">
                @csrf
                <div class="mb-3">
                ...
                <div class="mb-3">
                    <label class="form-label">Type</label>
                    <input type="text" name="type" class="form-control @error('type') is-invalid @enderror" value="{{ old('type') }}" placeholder="DMC, Airline, Transport, etc.">
                    @error('type')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
